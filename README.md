...
> **AI Support**
> If you want your AI assistant to work with BrowserCard decks on the file system, you may find a system (or project) prompt in file [SystemPrompt.md](./ai/SystemPrompt.md) and a complete agent skill in folder [browser-card](./ai/browser-card).
> 
...
**Stay organized.** The decks panel lists every deck stored in your browser - create, open, rename or delete them there, and optionally enable "remember last deck on reload" (off by default) so the designer reopens the deck you last worked on after a page reload, if it still exists. The card browser shows live wireframe thumbnails of all cards in the current deck and lets you add, duplicate, rename, reorder, nest (indent/outdent) and delete cards. Cards can be **nested** into a hierarchy purely for organization — each card stays a standalone object, navigation still walks the flattened depth-first order, and every card exposes a read-only `Path` (its ancestor names joined by `/`); deleting a card removes its whole subtree. Everything you do in edit mode is auto-saved to IndexedDB and protected by a 100-step undo/redo (Ctrl/Cmd+Z / Shift+Z).
...
| `my.Card.Index` | 0-based position of a card in the flattened (depth-first) card list (read/write; assigning moves the card, keeping it shown) — `my.Card.Index` gives the current card's index |
| `my.Card.Path` | computed, read-only: the card's ancestor names + own name, joined by `/` (e.g. `"Settings/Audio"`) |
