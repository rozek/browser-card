/**** nativeMonthInput - wraps a single-line <input type="month"> ****/
...
// also be given in "Text" (where editing writes it back, as "YYYY-MM").
...
/**** suggestionList - normalises Suggestions into a list of strings ****/
  ...
  const MonthStyle = `
    ...
    .bc-widget > input[type="month"]:invalid {
      border-color:red;
    }
    .bc-widget > input[type="month"]:read-only {
...
/**** actual behavior script ****/
  ...
  export default async function ({ on, my, html, dispatch, Configuration }) {
    styleRuleInjectedOnce('bc-nativemonthinput-style', MonthStyle)
    ...
    on('render', () => {
      ...
      const ListId      = ('bc-month-' + (my.Id ?? 'list'))
