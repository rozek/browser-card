/**** nativeTimeInput - wraps a single-line <input type="time"> ****/
...
// Configuration); editing writes the new value back to Value, as "HH:MM" or
// "HH:MM:SS". "invalid" forces the invalid state (on top
// of the native Minimum/Maximum check); "withSeconds" shows a seconds field.
// while focused, external Value changes do not disturb the input; on blur
...
/**** SuggestionList - normalises Suggestions into a list of strings ****/
  ...
  const TimeStyle = `
    ...
    .bc-widget > input[type="time"]:invalid {
      border-color:red;
    }
    .bc-widget > input[type="time"]:read-only {
      border:solid 1px #DDDDDD; border-radius:2px;
      background:#F0F0F0;
    }
    .bc-widget > input[type="time"]:disabled {
      cursor:not-allowed;
    }
  `
...
/**** actual behavior script ****/
  ...
  export default async function ({ on, my, html, dispatch, Configuration }) {
    injectStyleRuleOnce('bc-nativetimeinput-style', TimeStyle)
    ...
    on('render', () => {
      ...
      const ListId      = ('bc-time-' + (my.Id ?? 'list'))
