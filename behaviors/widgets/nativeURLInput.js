/**** nativeURLInput - wraps a single-line <input type="url"> ****/
...
// back). "invalid" forces the invalid state; "Pattern" is a regular expression
// the input must match to be valid. while focused, external Value/Text changes
...
/**** suggestionList - normalises Suggestions into a list of strings ****/
  ...
  const URLStyle = `
    ...
    .bc-widget > input[type="url"]:invalid {
      text-decoration:underline wavy red 1px;
    }
    .bc-widget > input[type="url"]:read-only {
      border:solid 1px #DDDDDD; border-radius:2px;
      background:#F0F0F0;
    }
    .bc-widget > input[type="url"]:disabled {
      cursor:not-allowed;
    }
  `
...
/**** actual behavior script ****/
  ...
  export default async function ({ on, my, html, dispatch, Configuration }) {
    styleRuleInjectedOnce('bc-nativeurlinput-style', URLStyle)
    ...
    on('render', () => {
      ...
      const ListId      = ('bc-url-' + (my.Id ?? 'list'))
