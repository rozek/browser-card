/**** nativeWeekInput - wraps a single-line <input type="week"> ****/
...
// also be given in "Text" (where editing writes it back, as "YYYY-Wnn").
...
/**** suggestionList - normalises Suggestions into a list of strings ****/
  ...
  const WeekStyle = `
    ...
    .bc-widget > input[type="week"]:invalid {
      border-color:red;
    }
    .bc-widget > input[type="week"]:read-only {
      border:solid 1px #DDDDDD; border-radius:2px;
      background:#F0F0F0;
    }
    .bc-widget > input[type="week"]:disabled {
      cursor:not-allowed;
    }
  `
...
/**** actual behavior script ****/
  ...
  export default async function ({ on, my, html, dispatch, Configuration }) {
    styleRuleInjectedOnce('bc-nativeweekinput-style', WeekStyle)
    ...
    on('render', () => {
      ...
      const ListId      = ('bc-week-' + (my.Id ?? 'list'))
